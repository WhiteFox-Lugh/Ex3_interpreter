...
- [x] Ex.3.3.2 [★★]
  ...
  テストケースに含まれていたような気がするので実装しなきゃダメっぽい。複数宣言するよりこれの実装の方がめんどくさかった気がする。
...
- [x] Ex.3.4.1 [必修]
  ...
  つまり`val f = <fun>`とかはOKで`val f = fun`とかはテストケース通らない。
...
- [x] Ex.3.4.2 [★★]
  ...
  は，20 を出力する．
  ```
  プリミティブ演算なので、`+`と`*`と`<`だけで大丈夫。ただし、Exercise3.2.3を通している人は`&&`と`||`も通さないとダメ（テストケースに`(Ex.3.4.2&Ex.3.2.3)`と書いてあるものが存在する）。
...
- [x] Ex.3.4.3 [★]
  ...
  といった簡略記法をサポートせよ．
  ```
  `fun x1 -> fun x2 -> ,,, -> fun xn = ...`とかと同じであることに気づけばOK。
  * 実装難易度: 1
  * 発想難易度: A
